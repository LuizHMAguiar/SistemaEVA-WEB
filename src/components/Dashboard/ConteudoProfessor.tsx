...
export function Conteudo(
    ...
    const [criarAvaliacoes] = useState<Avaliacao[]>();
    const [avaliacaoSelecionada, setAvaliacaoSelecionada] = useState<Avaliacao | null>(null);


    async function buscaCriarAvaliacoes(){
        const resultado = await fetch("https://sistemaeva-api.onrender.com/professor/avaliacoes/"+cpf_cnpjExibido);
    ...
    if (criarAvaliacoes == null && buscouAvaliacoes == false){
        buscaCriarAvaliacoes()
    }

    const [, setCod_Avaliacao] = useState("");
    ...
    async function IniciarAvaliacao(){ 
        const resultado = await fetch("https://sistemaeva-api.onrender.com/avaliacao/iniciar", { 
            ...
            body: "{\"cpf_professor\": \""+cpf_cnpjExibido+
    ...
    return (
        ...
        <div className="Conteudo">
             <div className="BuscarAvaliacoes">
                <input placeholder='Código da Aaliação' onChange={(e) => setCod_Avaliacao(e.target.value)}>
                </input>
            ...
            {!criarAvaliacoes&& <p>Nenhuma avaliação adicionada</p>}

            {criarAvaliacoes?.map((avaliacao) => (
                <div className="Avaliacao">
